...
export default function NovoFornecedorPage() {
  ...
  const [categoria, setCategoria] = useState<string>(""); // Segmento de fornecimento
  ...
  // 🔌 CONSULTA AUTOMÁTICA DE DOCUMENTO (INTEGRAÇÃO BACKEND)
  async function handleConsultarDocumento() {
    if (!documento) {
      alert(`Por favor, digite um ${tipoPessoa === "PJ" ? "CNPJ" : "CPF"} para realizar a consulta.`);
      return;
    }
    ...
    try {
      alert(`Consultando ${tipoPessoa}: ${documento} nas APIs de mercado...`);
      
      // Simulação do gatilho da sua API Express
      setTimeout(() => {
        if (tipoPessoa === "PJ") {
          setNome("DISTRIBUIDORA DE INSUMOS E LOGISTICA BRASIL S/A");
          setCep("04101-300");
          ...
          setNome("MARCOS ROBERTO DA SILVA JÚNIOR");
          setCategoria("Serviços");
          setObservacao("Prestador autônomo homologado via checagem cadastral.");
        }
        setConsultandoDocumento(false);
        alert(`Dados do ${tipoPessoa} importados com sucesso!`);
      }, 1500);

    } catch (error) {
      console.error(error);
      alert("Erro ao consultar documento. Preencha os campos manualmente.");
      setConsultandoDocumento(false);
    }
  }
  ...
  function handleSalvarFornecedor(e: React.FormEvent) {
    ...
    if (!nome || !documento || !email || !categoria || !cidade || !estado) {
    ...
    console.log("Enviando fornecedor estruturado para o Node.js:", novoFornecedor);
    alert("Fornecedor homologado e salvo com sucesso!");
    ...
    <div className="min-h-screen bg-slate-50 text-slate-700 flex flex-col antialiased">
      ...
      <main className="flex-1 p-4 md:p-6 max-w-3xl w-full mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900 tracking-tight">Novo Fornecedor</h1>
            <p className="text-xs text-slate-500 mt-0.5">Cadastre credores, distribuidores de mercadorias e prestadores operacionais.</p>
          </div>
        </div>

        {/* Formulário Principal */}
        <form onSubmit={handleSalvarFornecedor} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            >
              ...
              <div className="text-left">
                ...
                <p className="text-[10px] text-slate-400 font-semibold">Distribuidores, indústrias e atacados.</p>
              </div>
            </div>
            ...
            >
              ...
              <div className="text-left">
                <p className="text-xs font-bold text-slate-800">Pessoa Física (Autônomo)</p>
                <p className="text-[10px] text-slate-400 font-semibold">Prestadores individuais, consultores, freelancers.</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2 border-b border-slate-100 pb-2">
              ...
              Dados do Credor / Parceiro
            </h4>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              
              {/* CPF / CNPJ Dinâmico com consulta instantânea */}
              ...
              {/* Segmento / Categoria de Fornecimento */}
              <div className="flex flex-col gap-1.5">
                <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wide">Segmento de Atuação *</label>
                <select 
                  value={categoria}
                ...
                >
                  ...
                  <option value="Serviços">Prestação de Serviços / PJ</option>
                  <option value="Logística">Logística e Fretes</option>
                  <option value="Custos Fixos">Custos Fixos (Água, Luz, Internet)</option>
                  <option value="Ferramentas">Ferramentas e Licenças (SaaS)</option>
                </select>
              </div>
              ...
              <div className="flex flex-col gap-1.5 sm:col-span-2">
                <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wide">
                  {tipoPessoa === "PJ" ? "Razão Social / Nome Comercial *" : "Nome do Fornecedor *"}
                </label>
                <input 
                  type="text"
                  value={nome}
                  onChange={(e) => setNome(e.target.value)}
                  placeholder="Nome completo ou corporativo..."
              ...
              {tipoPessoa === "PJ" && (
                <div className="flex flex-col gap-1.5">
                  ...
                  <input 
                    ...
                    placeholder="Isento ou número fiscal"
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-xs font-medium text-slate-700 focus:outline-none focus:border-indigo-500 transition-colors"
                  />
                </div>
              )}
              ...
              {/* E-mail */}
              <div className="flex flex-col gap-1.5">
                <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wide">E-mail Comercial *</label>
              ...
              {/* Telefone */}
              <div className="flex flex-col gap-1.5">
                <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wide">Telefone de Contato</label>
                <div className="relative flex items-center">
          ...
          <div className="space-y-4 pt-2">
            <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2 border-b border-slate-100 pb-2">
              ...
              Localização / Sede Distribuidora
            </h4>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
              ...
              <div className="flex flex-col gap-1.5 sm:col-span-4">
                <div className="flex items-center gap-1.5">
                  ...
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wide">Observações / Dados Bancários Padrão</label>
                </div>
                <textarea 
                  value={observacao}
                  onChange={(e) => setObservacao(e.target.value)}
                  rows={3}
                  placeholder="Insira chaves Pix corporativas, prazos médios de entrega negociados, ou histórico acordado de fretes..."
